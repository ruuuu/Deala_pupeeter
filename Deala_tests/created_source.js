...
(async () => {
  ...
  await page.type("input[formcontrolname=email]", "TOMe45@mail.ru");//вводим в поле значение some45@mail.ru
  ...
  await page.type("input[type=password]", "Passw._90eertre");//вводим в поле значение Passw._90eertre
  await page.waitFor(1000);//ждем 1000милисек
 ...
 //перешли на главную станицу
  ...
  await page.$eval('a[href="/sources"]', el => el.click()); // жмем на раздел Sources
  await page.waitFor(3000);


  await page.$eval('button[class="mat-focus-indicator button gl-btn mat-flat-button mat-button-base mat-primary ng-star-inserted"]', el => el.click());//жмем на кнопку Add cources
  await page.waitFor(3000);//ждем 3000милисек


  await page.type("input[formcontrolname=title]", "name of source");//вводим в поле значение name of source
  await page.waitFor(1000);//ждем 1000милисек
